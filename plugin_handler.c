...
void Plugin_Load(char* name, size_t size)
{
	...
	/*
	pluginFunctions.plugins[i].OnInfoRequest = dlsym(lib_handle, "OnInfoRequest");


	pluginFunctions.plugins[i].OnEvent[PLUGINS_ONPLAYERDC] = dlsym(lib_handle, "OnPlayerDC");
	pluginFunctions.plugins[i].OnEvent[PLUGINS_ONPLAYERCONNECT] = dlsym(lib_handle, "OnPlayerConnect");
	pluginFunctions.plugins[i].OnEvent[PLUGINS_ONEXITLEVEL] = dlsym(lib_handle, "OnExitLevel");
	pluginFunctions.plugins[i].OnEvent[PLUGINS_ONMESSAGESENT] = dlsym(lib_handle, "OnMessageSent");
	pluginFunctions.plugins[i].OnEvent[PLUGINS_ONTENSECONDS] = dlsym(lib_handle, "OnTenSeconds");
	pluginFunctions.plugins[i].OnEvent[PLUGINS_ONCLIENTSPAWN] = dlsym(lib_handle, "OnClientSpawn");
	pluginFunctions.plugins[i].OnEvent[PLUGINS_ONCLIENTENTERWORLD] = dlsym(lib_handle, "OnClientEnterWorld");
	pluginFunctions.plugins[i].OnEvent[PLUGINS_ONTCPSERVERPACKET] = dlsym(lib_handle, "OnTcpServerPacketEvent");
	...
	*/
	...
	if((*pluginFunctions.plugins[i].OnInit)(mainFunctions)<0){
		...
		if(pluginFunctions.plugins[i].OnInfoRequest){
			...
			Com_PrintError("^1ERROR:^7 This plugin might not be compatible with this server version! Requested plugin handler version: %g, server's plugin handler version: %g. Unloading the plugin...\n",info.handlerVersion,PLUGIN_HANDLER_VERSION);
			Plugin_Unload(i);
			return;
		    }
