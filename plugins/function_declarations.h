...
#define CONVAR_T cvar_t
    ...
    void (QDECL *Plugin_ChatPrintf)(int slot, char *fmt, ...);                  // Print to player's chat (-1 for all)
    ...
    void (__cdecl *Plugin_SetPlayerUID)(unsigned int clientslot, unsigned int uid);     // Set player's UID
    unsigned int (__cdecl *Plugin_GetPlayerUID)(unsigned int clientslot);               // Get player's UID
    const char* (__cdecl *Plugin_GetPlayerGUID)(unsigned int clientslot);               // Get player's GUID
    void (__cdecl *Plugin_SetPlayerGUID)(unsigned int clientslot, const char* guid);    // Set player's GUID
    void (__cdecl *Plugin_SetPlayerNoPB)(unsigned int clientslot);                      // Turn off PunkBuster for a player
    int (__cdecl *Plugin_DoesServerUseUids)(void);                                      // Self explanatory
    void (__cdecl *Plugin_SetServerToUseUids)(int useuids);                             // Self explanatory
