...
qboolean SV_OversizeBanlistAlign(){
    ...
    if(current_banlist_size <= (current_banindex + 1) * sizeof(banList_t)){//Memory extension
        ...
        new_blist = realloc(psvs.banlist, newSize);
        if(new_blist){
            psvs.banlist = new_blist;
...
qboolean SV_ParseBanlist(char* line, time_t aclock, int linenumber){
    ...
    this = &psvs.banlist[current_banindex];
        ...
        if(NET_CompareBaseAdr(netadr, &this->remote)){
            ...
            if(realtime < this->timeout){
                ...
                if(this->expire == -1){
                    ...
                    int remaining = (int)(this->expire-realtime) +1; //in seconds (+1 for fixing up a display error when only some seconds are remaining)
...
void SV_PlayerAddBanByip(netadr_t remote, char *reason, int uid, int adminuid, int expire){		//Gets called by future implemented ban-commands and if a prior ban got enforced again - This function can also be used to unset bans by setting 0 bantime
    ...
    list->timeout = realtime + duration;
...
void SV_InitBanlist(){
    ...
    current_banindex = 0;
    psvs.banlist = realloc(NULL, current_banlist_size);//Test for NULL ?
    if(psvs.banlist){
        SV_LoadBanlist();
    }else{
